({
   ...
   {  
       ...
       //alert(component.get("v.DealRegistrationId"));
       ...
       action.setCallback(this, function(response) {
           ...
           if (state === "SUCCESS") {
               ...
               component.set("v.mapAccountNameToIRA",response.getReturnValue().mapAccountNameToIRAstatus);
               component.set("v.mapAccountNameToRoE",response.getReturnValue().mapAccountNameToRoEstatus);
               component.set("v.mapAccountNameToContacts",response.getReturnValue().mapAccountNameTolstContact);
               ...
               component.set("v.accountList", response.getReturnValue().lstParterAccountName);
               ...
               //alert(response.getReturnValue().objDR.Partner_Company_Name__r.Name + '-----' + response.getReturnValue().objDR.Partner_Contact__r.Name);
               
               var depnedentFieldMap = component.get("v.mapAccountNameToContacts");
               var ListOfDependentContact = depnedentFieldMap[response.getReturnValue().objDR.Partner_Company_Name__r.Name];
               ...
               component.set("v.contactList", dependentContacts);
                ...
                /* Start Set selected Partner Account */
               var optPartnerAcc = [];
               var partnerAccs = response.getReturnValue().lstParterAccountName;
               
               for (var i = 0; i < partnerAccs.length; i++) {
                   
                   if(partnerAccs[i] === response.getReturnValue().objDR.Partner_Company_Name__r.Name){
              ...
              component.find("selectAccountName").set("v.options", optPartnerAcc);
              /* End Set selected Partner Account */
               
               
                /* Start Set selected Partner Contact */
               var optPartnerContact = [];
               var partnerCons = component.get("v.contactList");
               
               for (var i = 0; i < partnerCons.length; i++) {
                   
                   if(partnerCons[i] === response.getReturnValue().objDR.Partner_Contact__r.Name){
                      
                       optPartnerContact.push({
                           class: "optionClass",
                           label: partnerCons[i],
                           value: partnerCons[i],
                           selected: "true" 
              ...
              component.find("selectPartnerContact").set("v.options", optPartnerContact);
              /* End Set selected Partner Contact */
   ...
   saveDealRegistrationController : function(component, event, helper) 
   {
           ...
           */
           ...
           action.setParams({ "objDR" : component.get("v.ObjDealRegistration"), 
                             ...
                             "selectedAccount" : component.find("selectAccountName").get("v.value"),
                             "selectedContact" : component.find("selectPartnerContact").get("v.value")});
    ...
    onAccountChange : function(component, event, helper) {    
       ...
       var depnedentFieldMap = component.get("v.mapAccountNameToContacts");
        ...
        var optSolutions = [];
        component.find("selectPartnerContact").set("v.options", optSolutions);
       if (controllerValueKey != '--None--') 
       {
           
           var ListOfDependentFields = depnedentFieldMap[controllerValueKey];
           helper.fetchDisContacts(component, ListOfDependentFields);  
       } 
       else 
       {
           component.set("v.contactList", ['--None--']);
       ...
       helper.accountDealAccessCheck(component, controllerValueKey); 
   },
