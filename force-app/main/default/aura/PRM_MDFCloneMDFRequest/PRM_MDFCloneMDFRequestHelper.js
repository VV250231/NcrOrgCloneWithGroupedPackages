({
    isValidate : function(component, event, helper)
    {
        ...
        if(!component.find("Activity_Name").get("v.value"))
        {
            ...
            if(component.get("v.currentActivityName") === component.find("Activity_Name").get("v.value")){
                component.find("Activity_Name").set("v.errors",[{message:"Activity name already exist. Please Enter other Name: "}]);
                submitREcord = false ;
            }
            else
        ...
        for (var indexVar = 0; indexVar < RowItemList.length; indexVar++) 
        {
            ...
            //alert('-------------RowItemList[indexVar].Estimated_Cost__c-----'+RowItemList[indexVar].Estimated_Cost__c);
            
            if(!$A.util.isEmpty(RowItemList[indexVar].Estimated_Cost__c))
            {
                var eCost = RowItemList[indexVar].Estimated_Cost__c + '';
                //alert('-------------RowItemList[indexVar].Estimated_Cost__c-----'+ eCost.match(numberRegex));
    ...
    saveRequest : function(component, event, helper){
        ...
        var action = component.get("c.saveCloneMDFDetail"); 
        action.setParams({"lstExpense" : component.get("v.requestExpenseList"), "objRequest" : component.get("v.request"),
        ...
        action.setCallback(this, function(response) {
            ...
            if (state === "SUCCESS") {
                ...
                //component.getEvent("DetailRequestEvt").setParams({"mdfRecordId" : response.getReturnValue(),"backFrom" : "Clone"}).fire();
                //alert(response.getReturnValue());
