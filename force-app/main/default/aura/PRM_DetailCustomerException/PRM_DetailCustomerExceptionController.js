({
	getCustomer : function(component, event, helper) {
        ...
        var action = component.get("c.getCustomerExceptionDetail");
     	var idParam = helper.getJsonFromUrl().ceid;
       	action.setParams({ "recId" : idParam});
     //   action.setParams({ "recId" : "a2uV00000015yEMIAY"});
        
        // Create a callback that is executed after the server-side action returns
        action.setCallback(this, function(response) {
            var state = response.getState();
            if (state === "SUCCESS") {
                component.set("v.ObjCustomerException", response.getReturnValue());
       ...
       /* $A.get("e.force:navigateToURL").setParams({ 
                   "url": "/edit-customer-exception?ceid=" + idParam
                }).fire();
         */
        var url1 = "/PartnerCentral/s/edit-customer-exception?ceid="+idParam;
    ...
    handleFilesChange: function(component, event, helper) {
     ...
     cSubmitForApproval : function(component, event, helper) {
        ...
        action.setCallback(this, function(a) {
            ...
            $A.get("e.force:navigateToURL").setParams({ 
                   "url": "/custom-exception-record?ceid=" + idParam
                }).fire();*/
        });
